...
> [!info] 藍色良品筆記：資訊塊
> 本筆記由紅色資訊源 [[01_紅色良品筆記_資訊源/CCNA-V1-CH2-15_red_note#1.3-0 官方主題分成六個邏輯領域|1.3-0 官方主題分成六個邏輯領域]] 拆出。
...
- 官方 CCNA exam topics list 將主題分成六個邏輯領域。
- 不過，對剛開始讀 CCNA 的學生來說，照著官方主題從上到下讀並不理想。
- 每位 CCNA instructor，包括作者自己，都會用不同方式安排書或課程的結構。
- 作者所知道的課程中，沒有任何一門是完全照 exam topics list 的順序教。
- 從高層次來看，本書兩冊會以下列順序涵蓋 exam domains。
- 1.0 Network Fundamentals 與 3.0 IP Connectivity。
- 2.0 Network Access（網路存取）。
- 3.0 IP Connectivity，再次深入。
- 4.0 IP Services（IP 服務）。
- 5.0 Security Fundamentals（安全基礎）。
- 6.0 Automation and Programmability（自動化與可程式化）。
...
### [[02_藍色良品筆記_資訊塊/1.3-1 本書各冊都涵蓋多個領域|1.3-1 本書各冊都涵蓋多個領域]]
